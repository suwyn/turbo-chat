...
class SqliteSubscriptionAdapter < ActionCable::SubscriptionAdapter::Async
  def initialize(*)
    super
    @sync = Mutex.new
  ...
  class Listener
    ...
    def listen
      @adapter.with_subscriptions_connection do |sqlite_conn|
        # ensure_table_exists(sqlite_conn)
    ...
    def ensure_table_exists(sqlite_conn)
      sqlite_conn.execute(<<-STATEMENT.squish)
